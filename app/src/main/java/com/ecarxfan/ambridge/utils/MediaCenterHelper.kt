...
class MediaCenterHelper(private val context: Context, private val sdk: SDKLoader) {
    companion object {
    ...
    fun onAPIReady(state: Boolean) {
        Log.e(TAG, "MediaCenterAPI onAPIReady called with $state")
        if (!state) {
            Log.e(TAG, "MediaCenterAPI not ready")
            return
        }
        ...
        Log.e(TAG, "Token retrieved ${token as String?}")
    ...
    fun onDestroy() {
        if (token != null) {
            Log.e(TAG, "Unregistering music client")
    ...
    private fun requestControl() {
        ...
        if (mediaControlClientAPI != null && controlToken != null) {
            ...
            Log.e(TAG, "request apple music media playback controlled by car")
        }
    }
}